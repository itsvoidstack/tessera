...
import {
  ...
  ChevronLeft, ChevronRight, Search, ChevronDown, ShieldCheck,
} from "lucide-react";
...
export default function AuditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const issues = useMemo(() => project?.auditIssues ?? [], [project]);
  ...
  if (!project) {
    return (
      <div className="flex flex-col items-center justify-center h-full py-24 text-center px-8">
        ...
        <button onClick={() => router.push("/dashboard")} className="px-4 py-2 bg-[#1a5c38] text-white rounded-lg text-xs font-medium hover:bg-[#145230] active:scale-[0.98] transition-all">
          Back to Dashboard
        </button>
      </div>
    );
  ...
  return (
    <div className="h-full flex flex-col bg-white dark:bg-[#0f172a] transition-colors">
      ...
      <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100 dark:border-gray-800 bg-white dark:bg-[#0f172a]">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
            Detailed security, code quality, and technical debt audit.
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-8 max-w-6xl mx-auto w-full">
        {issues.length === 0 ? (
          /* Empty / Pending State when backend audit hasn't returned issues */
          <div className="border border-gray-200 dark:border-gray-800 rounded-xl p-12 text-center bg-gray-50/40 dark:bg-gray-900/40 my-8">
            <div className="w-12 h-12 rounded-2xl bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 flex items-center justify-center mx-auto mb-4 shadow-2xs">
              <ShieldCheck size={24} className="text-[#1a5c38] dark:text-green-400" />
            </div>
            <h2 className="text-base font-semibold text-gray-900 dark:text-white mb-1">No Code Audit Issues Pending</h2>
            <p className="text-xs text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6 leading-relaxed">
              When connected to the FastAPI backend API, the Security and Code Quality agents will scan target source files and report vulnerabilities or maintainability risks here.
            </p>
            <div className="inline-flex items-center gap-2 text-xs border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 rounded-lg px-4 py-2 text-gray-600 dark:text-gray-300">
              Target Repo: <strong>{project.owner}/{project.repo}</strong>
            </div>
          </div>
        ) : (
          <div>
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="flex gap-0 border-b border-gray-100 dark:border-gray-800 flex-1">
                {TABS.map((tab) => (
                  <button
                    key={tab}
            ...
            {/* Table */}
            <div className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden bg-white dark:bg-gray-900 shadow-2xs">
              {paginated.length === 0 ? (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/60 text-[11px] font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                      ...
                      <th className="px-4 py-3 w-16">Line</th>
                            ...
                            >
                              <div className="flex-1 flex items-start gap-3 min-w-0 pr-4">
                                ...
                                <div className="min-w-0">
                                  <div className="text-xs font-semibold text-gray-900 dark:text-white truncate">{issue.title}</div>
                                  <div className="text-[11px] text-gray-500 dark:text-gray-400 truncate mt-0.5">{issue.description}</div>
                                </div>
                              </div>
                              ...
                              <div className="flex-1 min-w-0 pr-4">
                                <span className="text-xs font-mono text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-gray-800 border border-gray-100 dark:border-gray-700 rounded px-1.5 py-0.5 truncate inline-block max-w-xs">
                                  {issue.file}
                                </span>
                              </div>
                              ...
                              <div className="w-6 flex justify-end">
                                <ChevronDown
                                  size={14}
                                  className={`text-gray-400 transition-transform duration-200 ${isExpanded ? "rotate-180 text-gray-700 dark:text-white" : ""}`}
                                />
                              </div>
                            </div>

                            {/* Smooth Expanded Fix View */}
                            {isExpanded && (
                              <div className="px-12 py-4 bg-gray-50/70 dark:bg-gray-800/30 border-t border-gray-100 dark:border-gray-800 text-xs transition-all">
                                <div className="font-semibold text-gray-800 dark:text-gray-200 mb-1">Issue Description</div>
                                <p className="text-gray-600 dark:text-gray-400 mb-3 leading-relaxed">{issue.description}</p>
            ...
            {filtered.length > PAGE_SIZE && (
              <div className="flex items-center justify-between mt-4 text-xs">
                <span className="text-gray-400">
                  Showing {(page - 1) * PAGE_SIZE + 1}–{Math.min(page * PAGE_SIZE, filtered.length)} of {filtered.length} issues
                </span>
